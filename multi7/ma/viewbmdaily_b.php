...
//$getvalues=mysql_query("select ifnull(sum(amount),0) from bmdatabase_wlplaceout where bmdate='$enum' and subbmcode='$type' order by entriesdate asc");
	if($code=="FIX")
	{
		$getvalues=mysql_query("select ifnull(sum(amount),0)+(select IFNULL(sum(amount),0) from backup_plan where bmdate='$enum' and subbmcode='$type') from bmdatabase_wlplaceout where bmdate='$enum' and memberid like 'F%' and subbmcode = '$type' order by entriesdate asc");
	}
	else if(strtoupper($code)=="DOMIN")
	{
		$getvalues=mysql_query("select ifnull(sum(amount),0)+(select IFNULL(sum(amount),0) from backup_plan where bmdate='$enum' and subbmcode='$type') from bmdatabase_wlplaceout where bmdate='$enum' and memberid like '%A001%' and subbmcode = '$type' order by entriesdate asc");
	}
	else if(strtoupper($code)=="GROSS")
	{
		$getvalues=mysql_query("select ifnull(sum(amount),0)+(select IFNULL(sum(amount),0) from backup_plan where bmdate='$enum' and subbmcode='$type') from bmdatabase_wlplaceout where bmdate='$enum' and memberid like '%A003%' and subbmcode = '$type' order by entriesdate asc");
	}
	else if(strtoupper($code)=="LEVY")
	{
		$getvalues=mysql_query("select ifnull(sum(amount),0)+(select IFNULL(sum(amount),0) from backup_plan where bmdate='$enum' and subbmcode='$type') from bmdatabase_wlplaceout where bmdate='$enum' and memberid like '%A002%' and subbmcode = '$type' order by entriesdate asc");
	}
	else
	{
		$member_short=$code;

		$getvalues=mysql_query("select ifnull(sum(amount),0)+(select IFNULL(sum(amount),0) from backup_plan where bmdate='$enum' and subbmcode='$type') from bmdatabase_wlplaceout where bmdate='$enum' and memberid like '%$member_short%' order by entriesdate asc");
	}
...
echo "<td align='center'  ><span class='bn13text'>&nbsp;<b>$enum</b></span></td>";
	...
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a href='viewbmdaily.php?web=$type&month=$month&year=$year&deyt=$enum&code=$code'><font color='blue'>$negtopos_total</font></a></b></span></td>";
	} else { $postoneg_total = number_format((0-$results[0]),2);
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a href='viewbmdaily.php?web=$type&month=$month&year=$year&deyt=$enum&code=$code'><font color='red'>$postoneg_total</font></a></b></span></td>";
	}
echo "</tr>"; }
}
