...
//	$getvalues=mysql_query("select IFNULL(sum(amount),0),subbmcode from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode='$row_bmsql[0]' and entriesby='$weblogin'");
	$getvalues=mysql_query("select IFNULL(sum(amount),0)+(Select IFNULL(sum(amount),0) from backup_plan where subbmcode ='$row_bmsql[0]' and bmdate>='$enum_from' and bmdate<='$enum_to') from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode='$row_bmsql[0]' order by ref asc");
	
	//echo "select IFNULL(sum(amount),0)+(Select sum(amount) from backup_plan where subbmcode ='$row_bmsql[0]' and bmdate>='$enum_from' and bmdate<='$enum_to') from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode='$row_bmsql[0]' order by ref asc" . "<br>";
...
while ($row_getvalues = mysql_fetch_array($getvalues)) 
	...
	if ($row_getvalues[0]=="")
		...
		echo "<td align='center'><a href='viewbmdaily_b.php?web=$row_bmsql[0]&month=$month&year=$year'><span class='bn13text'><b><font color='blue'>$negtopos_total</</b></span></td>";
