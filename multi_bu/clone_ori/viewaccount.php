...
$datetime=date("Y-m-d H:i:s");
		...
		//	if(($projcode)&&($subcode)&&($bmname))
			if(($account))
				...
				$projcodesql=mysql_query("SELECT cpyaccount FROM cpyaccount WHERE cpyaccount='$account' order by no");
...
<tr>
 ...
 <td class="hed" ><span class="bn13text">&nbsp;<b>Details</b>&nbsp;</span></td>
	...
	$bmsql=mysql_query("SELECT cpyaccount,managerid FROM cpyaccount group by managerid, cpyaccount order by no asc");
...
$fundsql=mysql_query("SELECT ifnull(sum(amount), 0)+(SELECT ifnull(sum(amount), 0) FROM bmexpenses where cpyaccount='$cpyaccount')+(select ifnull(sum(hiddenfunds),0) from cpyaccount where cpyaccount='$cpyaccount') FROM bmdatabase_payment where cpyaccount='$cpyaccount' and bmdate>='2015-09-07'");
