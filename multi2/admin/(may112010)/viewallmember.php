...
<td height='10'><span class="bn12text">
 ...
 Page Size:<input class="searchformfiled" type="text" value="<?php echo $_POST["page_size"]; ?>" name="page_size" size="3">
			   <?php } else { ?>
 Page Size:<input class="searchformfiled" type="text" value="50" name="page_size" size="3">
 				 <?php } ?>
...
while ($row_memlist = mysql_fetch_array($result)) 
{
	...
	echo "<td align='center'><a style='font-family:Arial, Helvetica, sans-serif; vertical-align:middle; font-size:12px; font-weight:normal;  ' href='viewallmember.php?action=edit&memberid=$row_memlist[0]&page=".$_GET["page"]."&payge=".$_SESSION["page_size"]."&SortOrder=".$_SESSION["SortOrder"]."&SortBy=".$_SESSION["SortBy"]."&hiddenf=".$_SESSION["hiddenf"]."&searchID=".$_SESSION["searchID"]."&filter=".$_SESSION["filter"]."&mayneger=".$_SESSION["mayneger"]."' target='_self'>$row_memlist[0]</a></td>";// memberid
	echo "<td align='center'><span class='bn12text'>$row_memlist[1]</span></td>"; // membername
		
		//-=-= colors
		if ($total<0) {
		if ($_GET["page"]=="") $_GET["page"] = 1;
		echo "<td align='center' ><a style='font-family:Arial, Helvetica, sans-serif; vertical-align:middle; font-size:12px; color:red; font-weight:normal; ' href='javascript: openScript(\"viewmemberdetails.php?memberid=$row_memlist[0]&managerid=$row_memlist[10]&payge=$page_size&pagenum=". $_GET["page"] . "&delimit=".$_SESSION["delimiter"]."&SortBy=". $_POST["SortBy"] . "&searchID=". $_POST["searchID"] . "&filter=". $_POST["filter"] . "&mayneger=". $_POST["mayneger"] . "&hiddenf=". $_POST["hiddenf"] . "&SortOrder=". $_POST["SortOrder"] . "&renew=1\",1000,600)' target='_self'>$total</a></td>";
		}
		else {
		if ($_GET["page"]=="") $_GET["page"] = 1;
			echo "<td align='center' ><a style='font-family:Arial, Helvetica, sans-serif; vertical-align:middle; font-size:12px; color:blue; font-weight:normal; ' href='javascript: openScript(\"viewmemberdetails.php?memberid=$row_memlist[0]&managerid=$row_memlist[10]&payge=$page_size&pagenum=". $_GET["page"] . "&delimit=".$_SESSION["delimiter"]."&SortBy=". $_POST["SortBy"] . "&searchID=". $_POST["searchID"] . "&filter=". $_POST["filter"] . "&mayneger=". $_POST["mayneger"] . "&hiddenf=". $_POST["hiddenf"] . "&SortOrder=". $_POST["SortOrder"] . "&renew=1\",1000,600)' target='_self'>$total</a></td>";
...
/*	$super_total = $super_total + $total;
	...
	if ($itago==1) {
	echo "<td align='center'><a style='font-family:Arial, Helvetica, sans-serif; vertical-align:middle; font-size:12px; font-weight:normal;  ' href='viewallmember.php?action=edit&memberid=$row_memlist[0]' target='_self'>$row_memlist[0]</a></td>";// memberid
	echo "<td align='center'><span class='bn12text'>$row_memlist[1]</span></td>"; // membername
