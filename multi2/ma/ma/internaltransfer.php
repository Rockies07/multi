...
if ($_POST[submit]) {
	...
	switch($submit){
		case "Submit":
		if ($fromaccount=="")
			echo "<SCRIPT language=\"JavaScript\">alert('From Account Required!');</SCRIPT>";
		else
		if ($toaccount==$fromaccount)
		echo "<SCRIPT language=\"JavaScript\">alert('Cannot transfer from the same account!');</SCRIPT>";
		else {
		
		$memberlist=mysql_query("SELECT managerid FROM cpyaccount WHERE cpyaccount='$toaccount'");
		$memberlist2=mysql_query("SELECT managerid FROM cpyaccount WHERE cpyaccount='$fromaccount'");
		//echo "SELECT managerid FROM cpyaccount WHERE cpyaccount='$toaccount'" . "<br>";
		//echo "SELECT managerid FROM cpyaccount WHERE cpyaccount='$fromaccount'" . "<br>";
		$managerid1=mysql_result($memberlist,0,"managerid");
		$managerid2=mysql_result($memberlist2,0,"managerid");
		/*echo $managerid1 . "<br>";
		echo $managerid2 . "<br>";
		echo $weblogin . "<br>";*/
		if (strtoupper($weblogin)==strtoupper($managerid1) || strtoupper($weblogin)==strtoupper($managerid2)) {
...
mysql_query("insert into bmdatabase_payment values('','$converted_datetime','-','$toaccount','INT','$currency','$amount',UCASE('$weblogin'),'$datetime2','$remarks','','','$trans_ref')") or die(mysql_error());
		...
		{
			echo "<SCRIPT language=\"JavaScript\">alert('One Account must be his own!');</SCRIPT>";
		}
		}}
	}
	else {
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
	<tr>
	  <td align="center" colspan="7"><span class="maintitle">Internal Transfer </span></td>
	</tr>
	...
	<td align="left" nowrap class="text11">
	  ...
	  <td ><select name="fromaccount" class="searchformfiled" tabindex="2">
        ...
        <?php	$accountsql=mysql_query("SELECT cpyaccount,managerid FROM cpyaccount where managerid='$weblogin'");
				//$accountsql=mysql_query("SELECT cpyaccount,managerid FROM cpyaccount");
