...
mysql_query("update bmdatabase_wlplaceout set bmdate='$converted_datetime',subbmcode='$accounts',currencycode='$currency',amount='$amount',entriesby='$weblogin',remark='$remark',entriesdate='$webdatetime' where ref ='$refy'") or die(mysql_error());
	...
	if($allp!="1")
	{
		$counts = isset($_POST['check_listp_value']) ? count($_POST['check_listp_value']) : 0;
	}
	else
	{
	...
	if($allp!="1")
	{
		$datum = $_POST['check_listp_value'][$i];
	}
	else
	{
	...
	else
	{
		...
		if ($hiddenf==0 || $hiddenf==""){
			if ($method=='placeout') {
				...
				mysql_query("update bmdatabase_wlplaceout set pm='$value' WHERE ref = '$ref'");
			...
			//-=-=- get info from database
				...
				mysql_query("update bmdatabase_payment set pm='$value' WHERE ref = '$ref'");								
	...
	//	echo "update bmdatabase_payment set pm='0' WHERE ref in ($refcol) and not ref in ($not_to_uncheck_p)" . "<br>";
		...
		else // if none is seen
		{
			...
			mysql_query("update bmdatabase_wlplaceout set pm='0' WHERE ref in ($refcolw)");
			mysql_query("update bmdatabase_payment set pm='0' WHERE ref in ($refcolw)");
	...
	if($allh!="1")
	{
		$counts = isset($_POST['check_listh_value']) ? count($_POST['check_listh_value']) : 0;
	}
	else
	{
	...
	if($allh!="1")
	{
		$datum = $_POST['check_listh_value'][$i];
	}
	else
	{
	...
	//echo $datum . "<BR>";
		...
		if ($hiddenf==0 || $hiddenf==""){
			if ($method=='placeout') {
				mysql_query("update bmdatabase_wlplaceout set clr='$value' WHERE ref = '$ref'");
			...
			if ($method=='payment') {
			mysql_query("update bmdatabase_payment set clr='$value' WHERE ref = '$ref'");
...
$indivmem=mysql_query("SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$mum' and pm='0')) 
 as outstanding, (SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$mum' and pm='1')) 
 FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='1') as pmdue FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='0'");
 }
...
<td class="hed" width="35%"><span class="bn13text"><b>Remarks d</b></span></td>
<td class="hed"  width="5%"><span class="bn13text"><b><input type="checkbox" name="allp" value='1' onClick="checkAll(document.member_details['check_listp[]'],this)"></b></span></td>
 <td class="hed"  width="5%"><span class="bn13text"><b><input type="checkbox" name="allh" value='1' onClick="checkAll(document.member_details['check_listh[]'],this)" ></b></span></td>
...
//	echo $superamount . "<br>";
	...
	if ($pm==0) {
		if ($bilangin<>0)
		{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%placeout' id='check_p_$count' class='d$memberid' onclick='check_hide_p($count)'> <input type='hidden' value='$ref%placeout%0' name='check_listp_value[]' id='check_listp_$count'></td>";}
		else
		{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%payment' id='check_p_$count' class='d$memberid' onclick='check_hide_p($count)'> <input type='hidden' value='$ref%payment%0' name='check_listp_value[]' id='check_listp_$count'></td>";
	//	echo "1-$ref%payment" . "<br>";
		}
	}
	else {
		if ($bilangin<>0)
		{echo "<td align='center'><input type='checkbox' name='check_listp[]'  value='$ref%placeout' id='check_p_$count' checked='checked' class='d$memberid' onclick='check_hide_p($count)'> <input type='hidden' value='$ref%placeout%1' name='check_listp_value[]' id='check_listp_$count'></td>";}
		else
		{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%payment' id='check_p_$count' checked='checked' class='d$memberid' onclick='check_hide_p($count)'> <input type='hidden' value='$ref%payment%1' name='check_listp_value[]' id='check_listp_$count'></td>";
	...
	if ($clr==0) {
		if ($bilangin<>0)
		{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%placeout' id='check_h_$count' class='h$memberid' onclick='check_hide_h($count)'> <input type='hidden' value='$ref%placeout%0' name='check_listh_value[]' id='check_listh_$count'></td>";
	//	echo "$ref%placeout" . "<br>";
		}
		else
		{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%payment' id='check_h_$count' class='h$memberid' onclick='check_hide_h($count)'> <input type='hidden' value='$ref%payment%0' name='check_listh_value[]' id='check_listh_$count'></td>";
	//	echo "$ref%payment" . "<br>";
		}
	}
	else {
		if ($bilangin<>0)
		{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%placeout' id='check_h_$count' checked='checked' class='h$memberid' onclick='check_hide_h($count)'><input type='hidden' value='$ref%placeout%1' name='check_listh_value[]' id='check_listh_$count'></td>";
	//	echo "$ref%placeout" . "<br>";
		}
		else
		{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%payment' id='check_h_$count' checked='checked' class='h$memberid' onclick='check_hide_h($count)'><input type='hidden' value='$ref%payment%1' name='check_listh_value[]' id='check_listh_$count'></td>";
	//	echo "$ref%payment" . "<br>";
		}
	}
...
<td class='hed'  width='10%'><span class='bn13text'><b>Accounts</b></span></td>
<td class='hed' width='10%'><span class='bn13text'><b>Amount</b></span></td>
<td class='hed' width='35%'><span class='bn13text'><b>Remarks a</b></span></td>
<td class='hed'  width='5%'>
	<span class='bn13text'><b><input type='checkbox' name='allp' value='1' id='d$memberidadv' onClick=\"check_all(this.id,'d$memberidadv')\"></b></span>
</td> <td class='hed'  width='5%'>
	<span class='bn13text'><b><input type='checkbox' name='allh' value='1' id='h$memberidadv' onClick=\"check_all(this.id,'h$memberidadv')\"></b></span>
</td> <td class='hed'  width='10%'><span class='bn13text'><b>Action</b></span></td></tr>";
		}
		else { 
...
<td class='hed'  width='10%'><span class='bn13text'><b>Accounts</b></span></td>
<td class='hed' width='10%'><span class='bn13text'><b>Amount</b></span></td>
<td class='hed' width='35%'><span class='bn13text'><b>Remarks f</b></span></td>
<td class='hed'  width='5%'><span class='bn13text'><b><input type='checkbox' name='allp$mim' value='1' id='d$memberidadv' onClick=\"check_all(this.id,'d$memberidadv');\"></b></span></td> 
<td class='hed'  width='5%'><span class='bn13text'><b><input type='checkbox' name='allp$mim' value='1' id='h$memberidadv' onClick=\"check_all(this.id,'h$memberidadv');\"></b></span></td> <td class='hed'  width='10%'><span class='bn13text'><b>Action</b></span></td></tr>";
		//echo "<tr bgcolor='#888888'><td colspan='5' align='center'  class='hed'><span class='bn13text'><b>Sub Total</b></span></td><td class='hed'><span class='bn13text'><b><font color='red'>$super_dummy</font></b></span></td><td colspan='4' class='hed'></td></tr>";
		$superamount=0;
		$once=0;
		}
...
<script>
	function check_all(id_name,class_name)
	{	
	...
	function check_hide_h(count)
	{	
		var value=$("#check_listh_"+count).val();
		var value_arr=value.split("%");
		var current_value=value_arr[0]+"%"+value_arr[1];

		if($("#check_h_"+count).is(':checked'))
		{
			$("#check_listh_"+count).val(current_value+'%1');
		}
		else
		{
			$("#check_listh_"+count).val(current_value+'%0');
		}
	}

	function check_hide_p(count)
	{	
		var value=$("#check_listh_"+count).val();
		var value_arr=value.split("%");
		var current_value=value_arr[0]+"%"+value_arr[1];

		if($("#check_p_"+count).is(':checked'))
		{
			$("#check_listp_"+count).val(current_value+'%1');
		}
		else
		{
			$("#check_listp_"+count).val(current_value+'%0');
		}
	}
</script>
