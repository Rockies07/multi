...
class c_pagedetails extends CI_Controller 
{
	...
	function intake($type,$fromdate,$todate)
	{
		$this->load->helper(array('form'));
		$session_data = $this->session->userdata('userinfo');
		$userdata = $this->m_accounts->getinfo($session_data['uid']);
		...
		if($this->session->userdata('userinfo'))
		{
			switch($type)
			{
				case 'own':			$intake_array = $this->m_accounts->get_intake_own($session_data['uid'],$fromdate,$todate);
									break;
				case 'downline':	$intake_array = $this->m_accounts->get_intake_down($session_data['uid'],$fromdate,$todate);
									break;
			}
			$header_data = array(
					'intake_array' => $intake_array,
					'fromdate' => $fromdate,
					'todate' => $todate,
				);
			
			$this->load->view('v_intakedetails',$header_data); // default view after login is profile
		}
		else
		{
			//If no session, redirect to login page
