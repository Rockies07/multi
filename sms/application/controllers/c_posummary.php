	...
	function index()
	{
		...
		if($this->session->userdata('userinfo'))
		{
			...
			if($this->input->post('view'))
			{
				...
				{
					$downline_array = $this->m_accounts->get_downline_with_trans($session_data['uid'], $fromdate, $todate);
					$downline_type = $this->m_accounts->getdownlinetype($session_data['uid']);
					$placeout_data = $this->m_accounts->getmyplaceouttotal($fromdate,$todate,$session_data['uid']);
					$myintake_data = $this->m_accounts->getmyintaketotal($fromdate,$todate,$session_data['uid']);
				}
				else
				{
				...
				foreach($downline_array as $downlinedata) // calculate downline po - individual tax
				{
					...
					{
						$downplaceout_array = $this->m_accounts->getmyplaceouttotal($fromdate,$todate,$session_data['uid']);
					}
				...
				if(strlen($session_data['uid']) < 8)
				{
					$downline_indata = $this->m_accounts->getdownintaketotal($fromdate,$todate,$session_data['uid']);
				}
			...
			{
				case 6	:	$view_page = "v_posummary_agt";
							break;
				case 8	:	$view_page = "v_posummary_meb";
							break;
				default	:	$view_page = "v_posummary";
							break;
			}
			...
			$this->load->view($view_page); // default view after login is profile
		}
		else
		{
