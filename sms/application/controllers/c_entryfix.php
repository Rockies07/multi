...
class c_entryfix extends CI_Controller 
{
	...
	function index()
	{
		...
		if($this->session->userdata('userinfo'))
		{
			...
			if(strlen($session_data['uid']) == 8)
			{
				$pagedata = $this->m_accounts->getfixpage($session_data['uid']);
			}
	...
	function getfixpagedata($meb_id,$pageref)
	{
		...
		{
			//header('Content-Type: application/x-json; charset=utf-8');
			$fixpage_data = $this->m_accounts->getfixpagedata($meb_id,urldecode($pageref));
	...
	function delfixpagedata($meb_id,$pageref)
	{
		...
		{
			//header('Content-Type: application/x-json; charset=utf-8');
			$this->m_accounts->delfixpagedata($meb_id,$pageref);
			//echo(json_encode($fixpage_data));
		}
		else
		{
