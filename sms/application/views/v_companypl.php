...
<?PHP
						...
						foreach($coy_pl_array as $coy_pl_data)
						{
							...
							if($coy_pl_data['mas_id'] == $session_uid)
							{
...
<?PHP
								echo "<td><div align='right'>".number_format($coy_pl_data['share_co_amt'],2,'.',',')." [".number_format($coy_pl_data['share_co_perc'] / $drawcount)."]</div></td>";

								if($userdata['share_po'] > 0) // have po share.
								{
									echo "<td><div align='right'>".number_format($coy_pl_data['share_po_amt'],2,'.',',')." [".number_format($coy_pl_data['share_po_perc'] / $drawcount)."]</div></td>";
...
<?PHP
							...
							{
								$pl_share = $coy_pl_data['share_co_amt100'] + $coy_pl_data['share_po_amt100'];
								
								$master_nett = ($coy_pl_data['total_ticket'] - $coy_pl_data['total_strike']) - $pl_share;
?>
							<tr>
								<td><div align="center"><?PHP echo $coy_pl_data['mas_id'];?></div></td>
								<td><div align="right"><?PHP echo number_format($coy_pl_data['total_ticket'],2,'.',',');?></div></td>
								...
								<td><div align="right"><?PHP echo number_format($ticket_amount,2,'.',',');?></div></td>
								<td><div align="right"><?PHP echo number_format(($coy_pl_data['ticket_perc'] / $drawcount),4,'.',',');?> %</div></td>
								<td><div align='right'><?PHP echo number_format($coy_pl_data['share_co_amt100'],2,'.',',');?> [100]</div></td>

<?PHP
								if($userdata['share_po'] > 0) // have po share.
								{
?>
									<td><div align='right'><?PHP echo number_format($coy_pl_data['share_po_amt100'],2,'.',',');?> [100]</div></td>
<?PHP
								}
?>
...
?>
							<tr>
								<td><div align="center"><?PHP echo $bok_pl_data['bok_id']." (".$bok_pl_data['name'].")";?></div></td>
...
<?PHP
								if($userdata['share_po'] > 0) // correct the border
								{
?>
