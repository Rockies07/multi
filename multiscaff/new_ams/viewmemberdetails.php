...
<!-- 
function win(){
window.opener.location.href="viewallmember.php?payge=<?php echo $_SESSION["payge"]; ?>&page=<?php echo $_SESSION["pagenum"]; ?>&delimit=<?php echo $_SESSION["delimit"]; ?>&SortBy=<?php echo $_SESSION["SortBy"]; ?>&searchID=<?php echo $_SESSION["searchID"]; ?>&filter=<?php echo $_SESSION["filter"]; ?>&mayneger=<?php echo $_SESSION["mayneger"]; ?>&hiddenf=<?php echo $_SESSION["hiddenf"]; ?>&SortOrder=<?php echo $_SESSION["SortOrder"]; ?>";
self.close();
//-->
}
...
<table border="0" cellpadding="0" cellspacing="0" width="80%" align="center">
  ...
  echo "<a href='viewmemberdetails.php' target='_self'><img border='0' src='images/cancel.gif' align=left></a>";}
  else{
  echo"<span class='bn13textwhite15'><a href='viewmemberdetails.php?action=aditional' target='_self'><img border='0' src='images/new.jpg' align=left></a>&nbsp;&nbsp;&nbsp;";
...
if($action<>'edit'){
	$det = date('m/d/Y');
	echo "<form action='viewmemberdetails.php' method='post' name='addinfo' style='margin-bottom:0;' onsubmit='return validate_form(this);'>
...
/* echo "SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$memberid' and pm='0')) 
 as outstanding, (SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$memberid' and pm='1')) 
 FROM bmdatabase_payment where memberid = '$memberid' and NOT type = 'INT' and pm='1') as pmdue FROM bmdatabase_payment where memberid = '$memberid' and NOT type = 'INT' and pm='0'";*/
 }
 else {
$indivmem=mysql_query("SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$mum' and pm='0')) 
 ...
 FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='1') as pmdue FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='0'");
  /*echo "SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$mum' and pm='0')) 
 as outstanding, (SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$mum' and pm='1')) 
 FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='1') as pmdue FROM bmdatabase_payment where memberid = '$mum' and NOT type = 'INT' and pm='0'";*/
 }
