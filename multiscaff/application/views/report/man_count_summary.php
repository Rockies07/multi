...
<div id="content">
	...
	<h3 class="glyphicons table"><i></i>Man Count Summary Report</h3>
			...
			<div class="filter-bar">
				<?php echo form_open($action)?>
					...
					<div>
						<label>From:</label>
						<div>
							<input type="text" name="date_filter_from" id="date_filter_from" class="input-mini datetimepicker" value="<?php if($filter_date_from=="") echo date("m/d/Y"); else echo $filter_date_from;?>" style="width: 90px;" />
						</div>
						<label>To:</label>
						<div>
							<input type="text" name="date_filter_to" id="date_filter_to" class="input-mini datetimepicker" value="<?php if($filter_date_to=="") echo date("m/d/Y"); else echo $filter_date_to;?>" style="width: 90px;" />
						</div>
					</div>
					<div>
					...
					<table class="table table-bordered table-primary table-condensed">
						<thead>
							<tr>
								...
								?>
								<th class="center" style="vertical-align:middle; background: rgb(190, 58, 58); border-color: rgb(190, 58, 58);" class="center">Project</th>
								<?php
									for($x=0;$x<=$count;$x++)
									{
						...
						<tbody>
							<?php 
								foreach($man_count_report as $man_count_report_item)
								{
									...
									$project_id=$man_count_report_item['project_id'];
							?>
									<tr>
										<td class="center" style="vertical-align:middle;">
											<span style="text-decoration:underline; font-weight:bold;">
												<?php 
													echo anchor('report/man_count_summary_detail/'.$date_from.'/'.$date_to.'/'.$project_id, $project_name);
												?>
											</span>
										</td>
										<?php 
