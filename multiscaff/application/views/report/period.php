			...
			<div class="filter-bar">
				<?php echo form_open($action)?>
					...
					<div>
						<label>From:</label>
						<div>
							<input type="text" name="date_filter_from" id="date_filter_from" class="input-mini datetimepicker" value="<?php if($filter_date_from=="") echo date("m/d/Y"); else echo $filter_date_from;?>" style="width: 70px; vertical-align:top;" />
						</div>
						<label>To:</label>
						<div>
							<input type="text" name="date_filter_to" id="date_filter_to" class="input-mini datetimepicker" value="<?php if($filter_date_to=="") echo date("m/d/Y"); else echo $filter_date_to;?>" style="width: 70px; vertical-align:top;" />
						</div>
					</div>
					<div>
						<label>Project:</label>
							...
							<table class="table table-bordered table-primary table-condensed" style="border-top:solid 1px #d7d8da">
								<tbody>
									<tr>
										...
										<th width="50px" class="center">Safety</th>
									...
									<tr>
										...
										<?php
											if(strtotime($filter_date_from)<strtotime('2015-05-01'))
											{
											...
											{
												$total_spv_w_income=$supervisor_number;
											}
											$total_no_of_men_filter=$total_no_of_men_filter+$erector_number+$total_spv_w_income+$safety_number;
										?>	
										<td class="center"><?php echo $erector_number;?></td>
										<td class="center"><?php echo $storeman_number;?></td>
										<td class="center"><?php echo $safety_number;?></td>
										<td class="center">
											<?php 
												if(strtotime($filter_date_from)<strtotime('2015-05-01'))
												{
												 	echo ($supervisor_number-$total_spv_w_income);
												}
												else 
												{
													echo "0";
										...
										<td class="center">
											<?php 
												if ($total_spv_w_income!="") 
												{
													echo $total_spv_w_income;
												}
												else
												{
													echo 0;
										...
										<td class="center">
											<?php 
												...
												if(strtotime($filter_date_from)<strtotime('2015-05-01'))
												{
												 	$total_man_on_period=$total_no_of_men_filter+$driver_number+$storeman_number+$supervisor_number-$total_spv_w_income+$no_of_men_absent;
												}
												else 
												{
													$total_man_on_period=$total_no_of_men_filter+$driver_number+$storeman_number+$no_of_men_absent;
												}
												echo $total_no_of_men_filter."/".$total_strength;
				...
				<div class="widget-body">
					...
					<table class="table table-bordered table-primary table-condensed" style="border-top:solid 1px #d7d8da">
						...
						<tbody>
							...
							<?php
									$i++;
									$no_of_men=$report_absent_item['no_of_men'];
							...
							?>
										...
										<td class="center">
											<?php 
												...
												{
													$total_standby=$total_strength-$total_man_on_period;
													echo $no_of_men; //$total_standby;
												}
											?>
										</td>
								<?php
