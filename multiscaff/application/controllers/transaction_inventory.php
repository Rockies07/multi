...
class Transaction_inventory extends User_Access_Controller 
{
	private $limit=30;
	...
	function __construct()
	{
		...
		$this->load->model('transaction_inventory_model','transaction_inventory_model',TRUE);
	...
	{
		// Write to $content
		$data['transaction_inventory']=$this->transaction_inventory_model->get_data_list();
		
		if (empty($data['transaction_inventory']))
		{
			show_404();
		}
		...
		$this->load->view('transaction_inventory/index',$data);
	...
	{
		$data['action']=site_url('transaction_inventory/journal_statement');
		$data['link_back']=anchor('transaction_inventory/journal_statement', 'Back', array('class'=>'back'));
		...
		if($edit_id==0)
		{
			...
			$data['transaction_inventory']=$this->transaction_inventory_model->get_data_list($date_from,$date_to,$project,$site);
			...
			$this->load->view('template/sidelink');
			$this->load->view('transaction_inventory/statement',$data);
			$this->load->view('template/footer');
		}
		else
		{
			$get_transaction_detail=$this->transaction_inventory_model->get_by_id($edit_id);
			...
			$this->transaction_inventory_model->update($edit_id,$transaction_inventory);
			...
			$this->load->view('template/header',$data);
			$this->load->view('template/sidelink');
			$this->load->view('transaction_inventory/detail',$data);
			$this->load->view('template/footer');
		}
	}
	...
	{
		$data['action']=site_url('transaction_inventory/account_summary');
		$data['link_back']=anchor('transaction_inventory/account_summary', 'Back', array('class'=>'back'));
		...
		$data['account_summary']=$this->transaction_inventory_model->get_account_summary($date_from,$date_to,$project,$site);
		$data['title']=$this->access->get_site_name();
		$data['site_name']=$this->access->get_sys_name();	
		$data['quote']=$this->access->get_sys_motto();
		$data['project']=$this->project_model->get_data_list();
		...
		$this->load->view('template/header',$data);
		$this->load->view('template/sidelink');
		$this->load->view('transaction_inventory/account_summary',$data);
		$this->load->view('template/footer');
	}
	
	function inventory_requisition()
	{
		$data['action']=site_url('transaction_inventory/add_expenses');
		...
		else
		{
			...
			for($i=1;$i<=30;$i++)
			{
				...
				if($amount>0)
				{
					...
					$id=$this->transaction_inventory_model->insert($transaction_inventory);
				}
			}
			...
			redirect('transaction_inventory/index');
		}
	}

	function add_deposit()
	{
		$data['action']=site_url('transaction_inventory/add_deposit');
		...
		if($this->form_validation->run() === FALSE)
		{
			...
			$this->load->view('template/header',$data);
			$this->load->view('template/sidelink');
			$this->load->view('transaction_inventory/index_deposit',$data);
			$this->load->view('template/footer');
		}
		else
		{
			...
			redirect('transaction_inventory/add_deposit');
		}
	}
	
	function delete($id,$page)
	{
		//delete transaction_inventory
		$this->transaction_inventory_model->delete($id);
		redirect('transaction_inventory/'.$page);
	}
}

/* End of file transaction_inventory.php */
/* Location: ./application/controllers/transaction_inventory.php */