...
class Transaction_journal extends User_Access_Controller 
{
	...
	{
		$data['action']=site_url('transaction_journal/management');
		$data['link_back']=anchor('transaction_journal/management', 'Back', array('class'=>'back'));
		...
		if($edit_id==0)
		{
			...
			$this->load->view('template/sidelink');
			$this->load->view('transaction_journal/management',$data);
			$this->load->view('template/footer');
		}
		else
		{
	...
	function add_expenses()
	{
		...
		if($this->form_validation->run() === FALSE)
		{
			...
			$data['site_name']=$this->access->get_sys_name();	
			$data['quote']=$this->access->get_sys_motto();
			$data['site']=$this->site_model->get_data_list(1);
