...
$month = "0" . $month;
	...
	$bmsql=mysql_query("SELECT sum(amount) as total_amt, bmdate FROM bmdatabase_payment where MONTH(bmdate)='$month' and YEAR(bmdate)='$year' group by bmdate");
...
if ($row_bmsql['total_amt']>=0) { $bmamount = number_format(abs($row_bmsql['total_amt']),2);
echo "<td align='center'><span class='bn13text'><b><a href='viewbmmonthly_cash_detail.php?code=$bmcode$subbmcode&date=$get_date' style='color:blue'>$bmamount</a></b></span></td>";
} else { $bmamount = number_format((0-$row_bmsql['total_amt']),2);
echo "<td align='center'><span class='bn13text'><b><a href='viewbmmonthly_cash_detail.php?code=$bmcode$subbmcode&date=$get_date' style='color:red'>$bmamount</a></b></span></td>";
}
/*
echo "<td><span class='bn13text'><b>$row_bmsql[remark]</b></span></td>";*/

$super_total = $super_total + $row_bmsql['total_amt'];

}
	?>
	<tr>
	<td class="hed" style="text-align:right"><span class="bn13text">&nbsp;<b>Total</b>&nbsp;</span></td>
