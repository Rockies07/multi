...
//-=-=- get info from database
	...
	if ($ebm==$memaydi) {
		...
		$getmainmember = mysql_query("select if (0<(select count(memberid) from memberid where memberid='$ebm'),(select memberid from memberid where memberid='$ebm'),(select memberid from submembers where subid='$ebm')) as mainmember"); 
		$getmainmemberofremove = mysql_query("select if (0<(select count(memberid) from memberid where memberid='$memaydi'),(select memberid from memberid where memberid='$memaydi'),(select memberid from submembers where subid='$memaydi')) as mainmember"); 

		$maimmem=mysql_result($getmainmember,0,"mainmember"); // to 
		$maimmemofremove=mysql_result($getmainmemberofremove,0,"mainmember"); // from
		
		//=-=-=-= swap values
		if ($phim=='0') {
				$updatetotal = "update member_total set total=total+'$eamount', outstanding=outstanding+'$eamount' where memberid='$maimmem'"; 
				mysql_query($updatetotal);
				$updatetotal = "update member_total set total=total-'$amawnt', outstanding=outstanding-'$amawnt' where memberid='$maimmemofremove'"; 
				mysql_query($updatetotal);
			}
		else {
				$updatetotal = "update member_total set total=total+'$eamount', amountdue=amountdue+'$eamount' where memberid='$maimmem'"; 
				mysql_query($updatetotal);
				$updatetotal = "update member_total set total=total-'$amawnt', amountdue=amountdue-'$amawnt' where memberid='$maimmemofremove'"; 
				mysql_query($updatetotal);
			}	
		
		}
			
	$update_amount = "update bmdatabase_wlplaceout set memberid='$ebm', amount='$eamount',remark='$remark' where ref = '$getref'";
...
<div id="breadCrumb" class="breadCrumb module">
                    <ul>
                        <li>
                     	 ...
                     	 <li>
                            <b>View BM Monthly (All Accounts)</b>
                        </li>
...
for($x=1;$x<=$vilang;$x++) {
	...
	{
		$getvalues=mysql_query("select sum(amount),subbmcode,ref from bmdatabase_wlplaceout where bmdate='$enum' group by subbmcode order by entriesdate asc");
	}
	else
	{
		$getvalues="";
...
if (($_GET["type"]==$row[1] && $_GET["bmdate"]==$enum))
{
	if ($row[0]<0) { $negtopos_total = number_format(abs($row[0]),2);
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a name='$ref' href='viewbmmonthall.php?month=$month&year=$year#$super_ref'><font color='blue'>$negtopos_total</font></a></b></span></td>";
	} else { $postoneg_total = number_format((0-$row[0]),2);
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a name='$ref' href='viewbmmonthall.php?month=$month&year=$year#$super_ref'><font color='red'>$postoneg_total</font></a></b></span></td></tr>"; }
}
else
{
	if ($row[0]<0) { $negtopos_total = number_format(abs($row[0]),2);
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a name='$ref' href='viewbmmonthall.php?code=$code&web=$type&month=$month&year=$year&type=$row[1]&bmdate=$enum&action=star#$super_ref''><font color='blue'>$negtopos_total</font></a></b></span></td>";
	} else { $postoneg_total = number_format((0-$row[0]),2);
	echo "<td align='center'  ><span class='bn13text'>&nbsp;<b><a name='$ref' href='viewbmmonthall.php?code=$code&web=$type&month=$month&year=$year&type=$row[1]&bmdate=$enum&action=star#$super_ref''><font color='red'>$postoneg_total</font></a></b></span></td></tr>";
...
$month=$_GET["month"];*/
	...
	<table border='0' cellpadding='0' cellspacing='0' align='right' width='430' class='stats'>";
	echo "<tr><td align='center' class='hed'  bgcolor='#888888'><span class='bn13text'>&nbsp;<b>Member</b></span></td>";
	echo "<td align='center' class='hed'  bgcolor='#888888' ><span class='bn13text'>&nbsp;<b>Amount</b></span></td>";
	echo "<td align='center' class='hed'  bgcolor='#888888' ><span class='bn13text'>&nbsp;<b>Remark</b></span></td>";
	echo "<td align='center' class='hed'  bgcolor='#888888' ><span class='bn13text'>&nbsp;<b>Action</b></span></td></tr>";
	
	$sqlenumerate=mysql_query("select memberid,amount,remark,ref from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode='$subbmcode' order by ref asc");
...
//echo "row_enum[3]: " . $row_enum[3] . "==" . "ref: " . $ref . "<br>";
 if(($action=='edit')&&($row_enum[3]==$ref)){
  ...
  $bmsqldata=mysql_query("select memberid as member from memberid union select subid as member from submembers ORDER BY member");
	$bmsqlrows=mysql_num_rows($bmsqldata);
  echo ">>$bmsqlrows";
  echo"<option value='$row_enum[0]'>$row_enum[0]</option>";
  for($iy=0; $iy<$bmsqlrows; $iy++)
  {$iddata=mysql_result($bmsqldata,$iy,"member");
...
//	echo "2row_enum[3]: " . $row_enum[3] . "==" . "ref: " . $ref . "<br>";
	 if(($action=='edit')&&($row_enum[3]==$ref)){echo "<td align='center'><a href='viewbmmonthall.php?code=$code&web=$type&month=$month&year=$year&type=$row[1]&bmdate=$enum&ref=$row_enum[3]#$super_ref' target='_self'><img src='images/undo.gif' border='0' title='Undo'></a>&nbsp;<input type='image' src='images/save.gif' title='Save' name='update' ALT='Submit Form'></td>";}
		else{
		   echo "<td align='center' ><a href='viewbmmonthall.php?code=$code&web=$type&month=$month&year=$year&type=$row[1]&bmdate=$enum&action=edit&ref=$row_enum[3]#$super_ref' target='_self'><img src='images/edit.gif' border='0' title='Edit'></a></td>"; }
//	echo "";
	}
	//echo "</table><td></tr>";
	echo "</table>";
...
echo "<tr><td colspan='2' class='hid'></td><td align='center'  ><span class='bn13text'>&nbsp;<b><font color='blue'>$negtopos_total</font></b></span></td></tr>";
} else { $postoneg_total = number_format((0-$last_total),2);
echo "<tr><td colspan='2' class='hid'></td><td align='center'  ><span class='bn13text'>&nbsp;<b><font color='red'>$postoneg_total</font></b></span></td></tr>";
}
	?>
	</form>
</table>
