...
function KeyUpInPageNo(e) {
    evt = e || window.event;
	switch (evt.keyCode) 
	{
	  case 13:  // enter key, jump to 1st number
...
function BackupValues(NumId) {
    ...
    opDay = eval('document.PlaceBetForm.pDay' + iId);
...
function ChangeDay(NumId) {
	...
	switch(iCur)
	{
		case 0 :	document.getElementById("Day" + NumId).setAttribute("class", "day3");// 3 days selections
					break;
		case 1 :	document.getElementById("Day" + NumId).setAttribute("class", "day2");// 2 days selections
					break;
		case 2 :	document.getElementById("Day" + NumId).setAttribute("class", "day6");// 6 days selections
					break;
		case 3 :	document.getElementById("Day" + NumId).setAttribute("class", "day7");// 7 days selections
					break;
		case 4 :	document.getElementById("Day" + NumId).setAttribute("class", "day1");// 1 days selections
					break;
	}
...
function FocusInNum(NumId){

  if(NumId>1){
    var oDay = eval('document.PlaceBetForm.Day'+NumId);
    ...
    var oUpDay = eval('document.PlaceBetForm.Day'+(NumId-1));
	
    if(oNum.value == '' && oBigNum.value == '' && oSmlNum.value == ''){
      oDay.value = oUpDay.value; 
	  document.getElementById("Day"+NumId).setAttribute("class", "day"+oDay.value);
    }
  }
}
...
function ChangeValue(NumId) {
    ...
    //alert(document.getElementById("Day"+NumId).value);
...
//0:48(96)-------------9:57(105)
    switch (evt.keyCode) {
        ...
        case 40: // down key
            ...
            break
        case 106: // * key: change day
                if (isNaN(oNum.value)) {
                    oNum.value = oNum.value.substring(0, oNum.value.length - 1);
                }
                ChangeDay(NumId); // inside ChangeDay: update Day's pvalue
            break
        case 56: // shift + 8 = * key change day (laptop users)
            if (evt.shiftKey) {
                if (isNaN(oNum.value)) {
                    oNum.value = oNum.value.substring(0, oNum.value.length - 1);
                }
        ...
        case 82: // r key: roll
            ...
            if (oNum.value.length>1 && isNaN(oNum.value)) {
                oNum.value = oNum.value.substring(0, oNum.value.length - 1);
            }
			else{
				if(oNum.value=='/' || oNum.value=='r' || oNum.value=='R'){
...
// 106 == * 107 == + 109 == - 110 == . 111 == /
    switch (evt.keyCode) {
        ...
        case 82: // r key: roll
			...
			if(oSmlNum.value.length==1){
				oSmlNum.value=opSmlNum.value;
			}
			else{
				if (isNaN(oSmlNum.value)) {
					oSmlNum.value = oSmlNum.value.substring(0, oSmlNum.value.length - 1);
...
function CalculateAmount_Total() {
    var fResultOld = CalculateAmount_One(opNum.value, opBigNum.value, opSmlNum.value, opCmd.value, opDay.value);
    var fResultNew = CalculateAmount_One(oNum.value, oBigNum.value, oSmlNum.value, oCmd.value, oDay.value);
...
function ValidateForm(){
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							{
								nNumCount+=1;
								if (oSysEntry.value=='r' || oSysEntry.value=='R')
								{
								  //AddSysEntry(i);
								}
							}
	...
	if (bFormValid == true)
	{
		if (nNumCount>0)
		{
			if (confirm("Please note: Once the bets are submitted, you would not be able to change them!\nClick Ok to proceed with your bets, or Cancel to make changes."))
			{
				var postbalance = document.getElementById('Balance').value;
				var posttotal_amount = document.getElementById('TotalAmt').value;
				if (Number(posttotal_amount) > Number(postbalance))
				{
					alert("Bets exceed your remaining balance.\nPlease change your bets or contact your upline.");
					return false;
				}
				document.PlaceBetForm.submit();
			}
