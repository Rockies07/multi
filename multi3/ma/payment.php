...
<p align="center" class="maintitle">Multiple Payment Submission</p>
		...
		<table width="100%" border="0" cellpadding="4" cellspacing="0" bgcolor="#FFEDC7" align="center">
		  ...
		  <div class="demo">Date: <input type="text" id="inputDate" name="datetime" value="<?php echo date("m/d/Y"); ?>" readonly="true" style="width:80px">
            ...
            <td width="3%" colspan="2" rowspan="2" align="left" nowrap>&nbsp;&nbsp;&nbsp;Account:&nbsp;              <select name="selectAll" class="searchformfiled" onChange="javascript : selectAllType();" >
                ...
                <?php	$bmcodesql=mysql_query("SELECT cpyaccount FROM cpyaccount where managerid = '$weblogin'");
...
<tr bgcolor="#FFA800"> 
  ...
  <?php for ($i=1;$i<=30;$i++) { ?>
  	...
  	<td align="center"><input name="ID<?php echo $i; ?>" type="text" class="searchformfiled" tabindex="<?php echo $i; ?>" onBlur="up(this)" onKeyDown="if(event.keyCode==13) moveit(this);" size="10" maxlength="30"></td>
	<td align="center"><select name="eTypeAcc<?php echo $i; ?>" class="searchformfiled" onKeyDown="if(event.keyCode==13) event.keyCode=9;"><option value="">--Click--</option><?php	$bmcodesql=mysql_query("SELECT cpyaccount FROM cpyaccount where managerid = '$weblogin'");
				$bmcodesqlrow=mysql_num_rows($bmcodesql);
				for($count=0; $count<$bmcodesqlrow; $count++)
				{$data=mysql_result($bmcodesql,$count,"cpyaccount");
				echo "<option value='$data'>$data</option>";}
