...
//	echo $memberid;
	...
	$sql_get_member=mysql_query("SELECT memberid from memberid where memberid='$memberid' union select subid as memberid from submembers where memberid='$memberid'");
...
//echo "(SELECT ref,bmdate,entriesby,memberid,amount,type,cpyaccount,pm,clr,remark FROM bmdatabase_payment where memberid = '$memberid' and NOT type = 'INT' $hide_me) union (SELECT ref,bmdate,entriesby,memberid,amount,type,cpyaccount,pm,clr,remark FROM bmdatabase_payment where memberid in (select subid from submembers where memberid='$memberid' $hide_me)) union(SELECT ref,bmdate,entriesby,memberid,amount,type,subbmcode,pm,clr,remark FROM bmdatabase_wlplaceout where memberid = '$memberid' $hide_me) union (SELECT ref,bmdate,entriesby,memberid,amount,type,subbmcode,pm,clr,remark FROM bmdatabase_wlplaceout where memberid in (select subid from submembers where memberid='$memberid' $hide_me order by bmdate desc) and NOT type = 'INT' $hide_me) order by memberid,bmdate desc, ref desc";
	...
	if($once==0) {
	$mum=mysql_result($sql_get_member,0,"memberid");
	?>
...
<td class="hed"  width="5%"><span class="bn13text"><b><input type="checkbox" name="allp"  onClick="checkAll(document.member_details['check_listp[]'],this)"></b></span></td>
 ...
 <td class="hed"  width="10%"><span class="bn13text"><b>Action</b></span></td>
	...
	$bmsql=mysql_query("(SELECT ref,bmdate,entriesby,memberid,amount,type,cpyaccount,pm,clr,remark, 'payment' as ttype FROM bmdatabase_payment where memberid = '$get_member_id' and NOT type = 'INT' $hide_me) union
	(SELECT ref,bmdate,entriesby,memberid,amount,type,subbmcode,pm,clr,remark, 'placeout' as ttype FROM bmdatabase_wlplaceout where memberid = '$get_member_id' $hide_me)
	order by (case when memberid = '$get_member_id' then 0 else 1 end),memberid asc,bmdate desc, ref desc");
	...
	for($count=0;$count<$bmrow;$count++)
	{
		...
		if($get_member_id[0]=="P")
		{
			$bmcode=mysql_query("Select bmcode from bmcode where bmcode like '$get_member_id%'");
			$atsign=mysql_result($bmcode,0,"bmcode");
		}
		else
		{
			$get_type=mysql_query("Select membername from memberid where memberid like '$get_member_id%'");
			$atsign=mysql_result($get_type,0,"membername");
		}
		...
		$ttype=mysql_result($bmsql,$count,"ttype");
		...
		if ($pm==0) {
			if ($ttype=="placeout")
			{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%placeout' class='d$memberid'></td>";}
			else
			{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%payment' class='d$memberid'></td>";
			}
		}
		else {
			if ($ttype=="placeout")
			{echo "<td align='center'><input type='checkbox' name='check_listp[]'  value='$ref%placeout' checked='checked' class='d$memberid'></td>";}
			else
			{echo "<td align='center'><input type='checkbox' name='check_listp[]' value='$ref%payment' checked='checked' class='d$memberid'></td>";
			}
		}
		...
		if ($clr==0) {
			if ($ttype=="placeout")
			{
				echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%placeout' class='h$memberid'></td>";
			}
			else
			{
				echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%payment' class='h$memberid'></td>";
			}
		}
		else {
			if ($ttype=="placeout")
			{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%placeout' checked='checked' class='d$memberid'></td>";
		//	echo "$ref%placeout" . "<br>";
			}
			else
			{echo "<td align='center'><input type='checkbox' name='check_listh[]' value='$ref%payment' checked='checked' class='d$memberid'></td>";
		//	echo "$ref%payment" . "<br>";
			}
		}
		
		if (strtoupper($cpyaccount)!="CASH") {
	...
	$sql_get_subs=mysql_query("SELECT * from submembers WHERE memberid='$get_member_id' order by subid ASC");

	//echo "SELECT * from submembers WHERE memberid='$get_member_id' having subid not in ($member_shown)";
	while($data=mysql_fetch_array($sql_get_subs))
	{
		$get_submember=$data['subid'];
		$bmsql=mysql_query("(SELECT ref,bmdate,entriesby,memberid,amount,type,cpyaccount,pm,clr,remark, 'payment' as ttype FROM bmdatabase_payment where memberid = '$get_submember' and NOT type = 'INT' $hide_me) union
	(SELECT ref,bmdate,entriesby,memberid,amount,type,subbmcode,pm,clr,remark, 'placeout' as ttype FROM bmdatabase_wlplaceout where memberid = '$get_submember' $hide_me)
	order by (case when memberid = '$get_submember' then 0 else 1 end),memberid asc,bmdate desc, ref desc");

		$bmrow=mysql_num_rows($bmsql);

		if($bmrow>0)
		{
			$indivmem=mysql_query("SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$get_submember' and pm='0')) 
			 as outstanding, (SELECT (ifnull(SUM(amount),0) + (SELECT ifnull(SUM(amount),0) FROM bmdatabase_wlplaceout where memberid = '$get_submember' and pm='1')) 
			 FROM bmdatabase_payment where memberid = '$get_submember' and NOT type = 'INT' and pm='1') as pmdue FROM bmdatabase_payment where memberid = '$get_submember' and NOT type = 'INT' and pm='0'");
		...
		<tr bgcolor='#888888'><td ><span class='bn13text'>$data[subid]</span></td><td colspan='6' align='center' class='hedacheblack'><span class='bn13text'><b>$gtot $gout 	$gdue</b></span></td> <td><span class='bn13text'>D</span></td><td><span class='bn13text'>H</span></td><td>
	  <input name='hideme' type='hidden' value=''>
	  <input type='Submit' name='Save' value='Save' class='formButton' onClick='AjaxRefresh();'> 
	  </td></tr><tr >   ";
	  ?>
		...
		{
			$gtot = "Sub Total : <span class='bn13textskybluer'>$0.00</span>&nbsp;&nbsp;&nbsp;";
	
			$gout = "Outstanding : <span class='bn13textskybluer'>$0.00</span>&nbsp;&nbsp;&nbsp;";
			
			$gdue = "Amt Due : <span class='bn13textskybluer'>$0.00</span>";
			echo "
		<tr><td colspan='10' height='30px' class='mates'></td><tr>
		<tr bgcolor='#888888'><td ><span class='bn13text'>$data[subid]</span></td><td colspan='6' align='center' class='hedacheblack'><span class='bn13text'><b>$gtot $gout 	$gdue</b></span></td> <td><span class='bn13text'>D</span></td><td><span class='bn13text'>H</span></td><td>
	  <input name='hideme' type='hidden' value=''>
	...
	<td class='hed' width='10%'><span class='bn13text'><b>Amount</b></span></td>
	<td class='hed' width='35%'><span class='bn13text'><b>Remarks</b></span></td>
	<td class='hed'  width='5%'><span class='bn13text'><b><input type='checkbox' name='allp$mim' id='d$memberidadv' onClick=\"check_all(this.id,'d$memberidadv');\"></b></span></td> 
	<td class='hed'  width='5%'><span class='bn13text'><b><input type='checkbox' name='allp$mim' id='h$memberidadv' onClick=\"check_all(this.id,'h$memberidadv');\"></b></span></td> <td class='hed'  width='10%'><span class='bn13text'><b>Action</b></span></td></tr>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
			echo "<td align='center'><span class='bn13text'>$data[subid]</span></td>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
			echo "<td align='center'><span class='bn13text'>NO TRANS</span></td>";
			echo "<td align='center'><span class='bn13text'>0.00</span></td>";
			echo "<td align='center'><span class='bn13text'>MEMBER</span></td>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
			echo "<td align='center'><span class='bn13text'>-</span></td>";
		}
