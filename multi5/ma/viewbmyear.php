...
//	$bmsql=mysql_query("SELECT bmcode,subbmcode FROM bmcode");
	$bmsql=mysql_query("select bmcode from bmcode where year='both' UNION select subbmcode as bmcode from bmdatabase_wlplaceout where year(bmdate)='$selectmo' group by subbmcode having subbmcode in (select bmcode from bmcode) order by bmcode");

	//echo "select bmcode from bmcode where year='both' UNION select subbmcode as bmcode from bmdatabase_wlplaceout where year(bmdate)='$selectmo' group by subbmcode having subbmcode in (select bmcode from bmcode) order by bmcode";
...
//	$getvalues=mysql_query("select sum(amount) as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode in (select subbmcode from bmcode where bmcode = '$bmcode')");
	...
	if(strtoupper($bmcode)=="FIX")
	{
		$getvalues=mysql_query("select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and memberid like 'F%'");
	}
	else if(strtoupper($bmcode)=="DOMIN")
	{
		$getvalues=mysql_query("select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and memberid like '%A001%'");
	}
	else if(strtoupper($bmcode)=="GROSS")
	{
		$getvalues=mysql_query("select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and memberid like '%A003%'");
	}
	else if(strtoupper($bmcode)=="LEVY")
	{
		$getvalues=mysql_query("select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and memberid like '%A002%'");
	}
	else 
	{
		$member_short=$bmcode[0]."0";

		$getvalues=mysql_query("select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and memberid like '%$member_short%'");
	}
	

	//echo "select ifnull(sum(ifnull(amount,0)),0)+(select ifnull(sum(amount),0) from backup_plan where bmdate>='$enum_from' and bmdate<='$enum_to' and bmcode='$bmcode') as month_amount from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode in (select subbmcode from bmcode where bmcode = '$bmcode')<br>";

	$results_placeout=mysql_result($getvalues,0,"month_amount");

	$results=$results_placeout;
