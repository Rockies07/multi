...
//$getvalues=mysql_query("select memberid,amount,entriesdate,ref from bmdatabase_wlplaceout where bmdate>='$enum_from' and bmdate<='$enum_to' and subbmcode='$type' and entriesby='$weblogin' order by entriesdate asc");
	if($code=="FIX")
	{
		$getvalues=mysql_query("select memberid,amount,entriesdate,ref,remark from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode like '$type%' and memberid like 'F%' order by ref asc");
	}
	else if(strtoupper($code)=="DOMIN")
	{
		$getvalues=mysql_query("select memberid,amount,entriesdate,ref,remark from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode like '$type%' and memberid like '%A001%' order by ref asc");
	}
	else if(strtoupper($code)=="GROSS")
	{
		$getvalues=mysql_query("select memberid,amount,entriesdate,ref,remark from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode like '$type%' and memberid like '%A003%' order by ref asc");
	}
	else if(strtoupper($code)=="LEVY")
	{
		$getvalues=mysql_query("select memberid,amount,entriesdate,ref,remark from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode like '$type%' and memberid like '%A002%' order by ref asc");
	}
	else
	{
		$member_short=$code[0];

		$getvalues=mysql_query("select memberid,amount,entriesdate,ref,remark from bmdatabase_wlplaceout where bmdate='$deyt' and subbmcode like '$type%' and memberid like '%$member_short%' order by ref asc");
	}
